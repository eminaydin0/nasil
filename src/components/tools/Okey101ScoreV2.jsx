  ...
  if (!isGameStarted) {
    return (
      <div className="h-full flex items-center justify-center bg-gray-50 p-4 sm:p-6 overflow-y-auto">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 md:p-8 max-w-2xl w-full my-4">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                {players.map((player, idx) => (
                  <input
                    ...
                    className="p-2.5 sm:p-3 text-sm sm:text-base border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:outline-none"
                  />
                ))}
              </div>
            </div>
          ...
          <button
            onClick={startGame}
            className="w-full py-3 sm:py-4 bg-indigo-600 text-white rounded-lg font-bold text-base sm:text-lg hover:bg-indigo-700 transition-colors shadow-md"
          >
            Oyunu Başlat
          </button>
        </div>
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      {isGameFinished && rounds.length > 0 && winnerName && (
        <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-4 sm:px-6 py-3 sm:py-4 shadow-lg">
      ...
      {/* Table Header */}
      <div className="border-b-2 border-gray-300 bg-gradient-to-r from-gray-50 to-white overflow-x-auto">
        <table className="w-full min-w-[500px]">
          <thead>
            <tr>
              <th className="p-2 sm:p-3 md:p-4 w-12 sm:w-16 md:w-20 text-center font-bold text-xs sm:text-sm md:text-base text-gray-700 border-r-2 border-gray-300 bg-gray-100/50">El</th>
              {isPartners ? (
                <>
                  <th className="p-2 sm:p-3 md:p-4 text-center">
                    <input
                      ...
                      className="w-full text-center font-bold text-sm sm:text-base md:text-lg text-gray-900 bg-transparent border-b-2 border-gray-400 focus:border-indigo-600 focus:outline-none py-1 sm:py-1.5 md:py-2 placeholder:text-gray-400"
                      placeholder="1. Takım"
                    />
                  </th>
                  <th className="p-2 sm:p-3 md:p-4 text-center">
                    <input
                      ...
                      className="w-full text-center font-bold text-sm sm:text-base md:text-lg text-gray-900 bg-transparent border-b-2 border-gray-400 focus:border-purple-600 focus:outline-none py-1 sm:py-1.5 md:py-2 placeholder:text-gray-400"
                      placeholder="2. Takım"
                    />
                  </th>
                </>
              ...
              <th className="p-2 sm:p-3 md:p-4 w-12 sm:w-14 md:w-16 text-center">
                <button
                  onClick={resetGame}
                  className="text-gray-500 hover:text-red-600 transition-colors p-1.5 sm:p-2 hover:bg-red-50 rounded-lg"
                  title="Sıfırla"
                >
      ...
      <div className="flex-1 overflow-auto bg-gray-50">
        {rounds.length === 0 ? (
          <div className="flex items-center justify-center h-full text-gray-400">
            <p className="text-xs sm:text-sm">Henüz el girilmedi</p>
          </div>
        ) : (
          <div ref={historyRef} className="p-1 sm:p-2 overflow-x-auto">
            <table className="w-full min-w-[500px]">
              <tbody>
                {rounds.map((roundData, rIdx) => {
                    ...
                    return (
                      <tr key={rIdx} className={`border-b border-gray-200 hover:bg-gray-50/50 transition-colors ${isPenalty ? 'bg-orange-50/40' : 'bg-white'}`}>
                        <td className="p-2 sm:p-3 md:p-4 w-12 sm:w-16 md:w-20 text-center font-semibold text-xs sm:text-sm text-gray-600 border-r-2 border-gray-200 bg-gray-50/50">
                        ...
                        <td className="p-2 sm:p-3 md:p-4 text-center">
                          <button 
                            ...
                            className="text-gray-400 hover:text-red-600 transition-colors p-1 sm:p-1.5 hover:bg-red-50 rounded"
                            title="Eli Sil"
                          >
              ...
              <tfoot>
                <tr className="bg-gradient-to-r from-gray-100 to-gray-50 border-t-2 border-gray-400">
                  <td className="p-2 sm:p-3 md:p-4 w-12 sm:w-16 md:w-20 text-center font-bold text-xs sm:text-sm md:text-base text-gray-700 border-r-2 border-gray-300">Toplam</td>
                  {isPartners ? (
                    <>
      ...
      <div className="border-t border-gray-200 bg-white p-2 sm:p-3 md:p-4">
            ...
            <tbody>
              <tr>
                {isPartners ? (
                  <>
                    <td className="p-2 sm:p-3">
                      <input
                        ...
                        className="w-full p-2 sm:p-2.5 md:p-3 text-center font-bold text-base sm:text-lg md:text-xl border-2 border-gray-300 rounded-lg focus:border-indigo-600 focus:ring-2 focus:ring-indigo-200 focus:outline-none transition-all"
                      />
                      <div className="flex gap-1 sm:gap-1.5 mt-2 sm:mt-2.5">
                        <button onClick={() => setPreset(0, PRESETS.BITTI)} className="flex-1 px-1.5 sm:px-2 md:px-2.5 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition-colors shadow-sm">Bitti</button>
                        <button onClick={() => setPreset(0, PRESETS.OKEY_BITTI)} className="flex-1 px-1.5 sm:px-2 md:px-2.5 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">Okey</button>
                    ...
                    <td className="p-2 sm:p-3">
                      <input
                        ...
                        className="w-full p-2 sm:p-2.5 md:p-3 text-center font-bold text-base sm:text-lg md:text-xl border-2 border-gray-300 rounded-lg focus:border-purple-600 focus:ring-2 focus:ring-purple-200 focus:outline-none transition-all"
                      />
                      <div className="flex gap-1 sm:gap-1.5 mt-2 sm:mt-2.5">
                        <button onClick={() => setPreset(1, PRESETS.BITTI)} className="flex-1 px-1.5 sm:px-2 md:px-2.5 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition-colors shadow-sm">Bitti</button>
                        <button onClick={() => setPreset(1, PRESETS.OKEY_BITTI)} className="flex-1 px-1.5 sm:px-2 md:px-2.5 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">Okey</button>
                  ...
                  currentScores.map((score, idx) => (
                    <td key={idx} className="p-2 sm:p-3">
                      ...
                      />
                      <div className="flex gap-1 sm:gap-1.5 mt-2 sm:mt-2.5">
                        <button onClick={() => setPreset(idx, PRESETS.BITTI)} className="flex-1 px-1.5 sm:px-2 md:px-2.5 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition-colors shadow-sm">Bitti</button>
                ...
                <td className="p-2 sm:p-3">
                  <button
                    onClick={addRound}
                    className="w-full py-2 sm:py-2.5 md:py-3 bg-gray-900 text-white rounded-lg font-bold hover:bg-gray-800 transition-all shadow-md hover:shadow-lg"
                  >
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-3 mt-3 sm:mt-4 pb-1 sm:pb-2">
          <label className="flex items-center gap-1.5 sm:gap-2 cursor-pointer px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg hover:bg-gray-50 transition-colors">
            <input 
              type="checkbox" 
